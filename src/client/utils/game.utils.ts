...
function makeAIInput(AI: AI_class, socket: WebSocket) {
  ...
  const current_position = {
    x: g_game_state.paddle2.x,
    y: g_game_state.paddle2.y,
  };

  if (
    g_game_state.score1 != AI.knownScore.p1 ||
    g_game_state.score2 != AI.knownScore.p2
  ) {
  ...
  if (date.getTime() - AI.lastCheck.getTime() >= 1000) {
    ...
    if (ball_speed.vx <= 0) {
      ...
      AI.expectedHitpoint = linear_extrapolation(
        ball_position,
        ball_speed,
        current_position.x,
      );
  ...
  if (
    Number(AI.expectedHitpoint) <
      Number(current_position.y + g_game_state.paddle2.height / 2) &&
    Number(current_position.y + g_game_state.paddle2.height / 2) -
      Number(AI.expectedHitpoint) >
      g_game_state.paddle2.height / 2
  ) {
    socket.send(JSON.stringify({ playerId: 2, action: "up" }));
  } else if (
    Number(AI.expectedHitpoint) >
...
function linear_extrapolation(
  ball_position: { x: number; y: number },
  ball_speed: { vx: number; vy: number },
  x_hitpoint: number,
) {
  const dt = 1 / 60;
...
export function startPongInContainer(
  ...
  title.className =
    'text-2xl font-["Orbitron"] text-white text-center mt-8 mb-4';
  ...
  bg_canvas.className =
    "border-2 border-black rounded-md shadow-[4.0px_5.0px_0.0px_rgba(0,0,0,0.8)]";
  ...
  const wsHandler = createGameWebSocket(
    gameId,
    leftPlayer,
    rightPlayer,
    onFinish,
  ...
  socket.addEventListener("message", (event) => {
    try {
      const data = JSON.parse(event.data);
      if (
        (mode === "duo-online" || mode === "tournament-online") &&
        data.playerNames &&
        data.playerNames[1] &&
        data.playerNames[2]
      ) {
  ...
  socket.addEventListener("message", (event) => {
    try {
      ...
      if (isGameState) {
        if (
          g_game_state &&
          (data.score1 != g_game_state.score1 ||
            data.score2 != g_game_state.score2)
        ) {
...
export function showGameOverOverlay(
  ...
  if (mode === "local") {
    const replay = CommonComponent.createStylizedButton(
      `${language_obj["Ingamepage_replay_button"]}`,
      "blue",
    );
    replay.onclick = () => {
      window.dispatchEvent(new Event("app:close-sockets"));
...
function showCountdown(message: string) {
  ...
  (
    overlay.querySelector("#game-countdown-panel") as HTMLDivElement
  ).textContent = message;
  overlay.style.display = "flex";
}
