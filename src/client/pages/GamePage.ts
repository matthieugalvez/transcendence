...
function createMainWrapper(): HTMLDivElement {
  ...
  wrapper.className =
    "main-content responsive-container flex items-center justify-center relative";
...
function createGameControls(
  ...
  const tourBtn = CommonComponent.createStylizedButton(
  ...
  ("../assets/gameimg/screen-pongGame.png");
...
export async function renderPongGamePage() {
  ...
  initialTitle.className =
    'text-2xl font-["Orbitron"] text-white text-center mb-4';
      ...
      const settingsBar = GameSettingsComponent.render("solo", {
        onStartGame: async () => {
          ...
          const res = await fetch("/api/game/start", {
            ...
            body: JSON.stringify({
              difficulty: GameSettingsComponent.currentDifficulty,
            }),
          });
            ...
            const newSettingsBar = GameSettingsComponent.render("duo-local", {
              onStartGame: async () => {
                ...
                pongHandle = startPongInContainer(
                  ...
                  (winnerId) => {
                    const titleText =
                      gameContainer.querySelector("h2")!.textContent!;
                    const [name1, name2] = titleText.split(" vs ");
                    const winnerName = winnerId === 1 ? name1 : name2;
                    showGameOverOverlay(
                      gameContainer,
                ...
                const gameSettingsBar = GameSettingsComponent.render(
                  "duo-start",
                  {
                    onPauseGame: () => {
                      pauseState.value = !pauseState.value;
