...
export async function renderChatPage() {
  ...
  const receiver_name = document.URL.substring(
  ...
  const block_button = CommonComponent.createStylizedButton(
    `${language_obj["Block"]}`,
    "red",
  );
  ...
  const unblock_button = CommonComponent.createStylizedButton(
    `${language_obj["Unblock"]}`,
    "red",
  );
  ...
  if (friendship_status.status === "blocked") {
    ...
    if (friendship_status.status === "none") {
      ...
      const reject_button = CommonComponent.createStylizedButton(
        `${language_obj["Reject_request"]}`,
        "gray",
      );
    ...
    if (
      ...
      for (const message_box of messages_list) {
        if (message.id == message_box.title) {
          if (!message.deleted) {
            message_box.firstElementChild.firstElementChild.style.fontStyle =
              "normal";
...
function makeEditPromptArea(message: any, box_text: Element) {
  ...
  const send_button = CommonComponent.createStylizedButton(
    `${language_obj["Send"]}`,
    "blue",
  );
  ...
  const close_button = CommonComponent.createStylizedButton(
    `${language_obj["Close"]}`,
    "red",
  );
