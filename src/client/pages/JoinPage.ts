...
export async function renderJoinPage(params: {
  ...
  document.title =
    mode === "duo" ? "Pong - Online Duo" : "Pong - Tournoi Online";
  ...
  wrapper.className =
    "main-content responsive-container flex min-h-screen w-full items-center justify-center relative";
  document.body.appendChild(wrapper);

  // Create flex container for game and settings
  const gameAndSettingsContainer = document.createElement("div");
  gameAndSettingsContainer.className =
    "flex items-start justify-center gap-6 w-full max-w-7xl mx-auto";
  ...
  const canvas = gameContainer.querySelector(
    "canvas",
    ...
    try {
      ...
      if (data.type === "end") {
        ...
        if (data.reason === "timeout") {
          ...
          CommonComponent.showMessage(
            `${language_obj["Game_ended"]} ${data.message}`,
            "warning",
          );
        }
      ...
      if (data.type === "error") {
        ...
        if (data.error === "already_joined") {
          ...
          waiting.className = waiting.className.replace(
            "text-white",
            "text-red-500",
          );
        ...
        CommonComponent.showMessage(
          `❌ ${language_obj["Game_error"]} ${data.error}`,
          "error",
        );
        setTimeout(() => {
          window.dispatchEvent(new Event("app:close-sockets"));
      ...
      if (data.type === "error" && data.error === "invite_expired") {
        CommonComponent.showMessage(
          `❌ ${language_obj["Your_invite_expired"]}`,
          "error",
        );
        setTimeout(() => {
          window.dispatchEvent(new Event("app:close-sockets"));
        ...
        if (mode === "tournament") {
          ...
          if (data.type === "matchStart") {
            ...
            if (lastWinner) {
              ...
              winnerMsg.className =
                'font-["Canada-big"] uppercase mb-4 text-white text-2xl';
        ...
        if (
          ...
          if (mode === "duo") {
            ...
            bothPlayersConnected =
              !!data.connectedPlayers && data.connectedPlayers.length === 2;
          ...
          if (bothPlayersConnected && hasHadDisconnection && data.isPaused) {
            if (playerId === 1 && !resumeAlertShown) {
              CommonComponent.showMessage(
                `${language_obj["Both_players_back"]}`,
                "info",
              );
              renderSettingsBar();
          ...
          if (mode === "duo" && (playerId === 1 || playerId === 2)) {
            if (data.isRunning) {
              ...
              waiting.textContent = bothPlayersConnected
                ? playerId === 1
                  ? `${language_obj["Click_start_game"]}`
                  : `${language_obj["Waiting_for_host"]}`
            ...
            if (data.isRunning) {
              ...
              const newSettingsBar = GameSettingsComponent.render("duo-guest", {
                onPauseGame: () => {
                  ...
                  pongHandle?.socket.send(
                    JSON.stringify({
                      action: pauseState.value ? "pause" : "resume",
                    }),
                  );
                },
  ...
  const cleanupGameOnLeave = async () => {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        ...
        console.warn(
          "Network error during game cleanup - server may be unreachable",
        );
      } else {
      ...
      if (playerId === 1) {
        ...
        const settingsBar = GameSettingsComponent.render("duo-online", {
          ...
          onPauseGame: () => {
            pauseState.value = !pauseState.value;
            pongHandle?.socket.send(
              JSON.stringify({ action: pauseState.value ? "pause" : "resume" }),
            );
          },
          onRestartGame: () => window.location.reload(),
          onDifficultyChange: (difficulty) => {
            if (
              pongHandle &&
              pongHandle.socket &&
              pongHandle.socket.readyState === pongHandle.socket.OPEN
            ) {
              pongHandle.socket.send(
                JSON.stringify({ action: "difficulty", difficulty }),
              );
            }
          },
        });
