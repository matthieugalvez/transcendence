...
export async function renderTournamentPage() {
      ...
      onStartGame: () => {
        ...
        const newBar = GameSettingsComponent.render("duo-start", {
          onPauseGame: () => {
            ...
            if (
              currentMatchSocket &&
              currentMatchSocket.readyState === currentMatchSocket.OPEN
            ) {
              currentMatchSocket.send(
                JSON.stringify({
  ...
  previewImg.className =
    "w-[800px] h-[610px] opacity-70 border-2 border-black rounded-md shadow-[4.0px_5.0px_0.0px_rgba(0,0,0,0.8)] transition-all";
...
export async function launchTournament(
  ...
  async function playMatch(i: number) {
    ...
    if (
      currentMatchSocket &&
      currentMatchSocket.readyState === WebSocket.OPEN
    ) {
    ...
    const pongHandle = startPongInContainer(
      ...
      async (winnerId, score1, score2) => {
        ...
        if (i === matchups.length - 1) {
          try {
            console.log(
              "typeof GameService.createTournament:",
              typeof GameService.createTournament,
            );
