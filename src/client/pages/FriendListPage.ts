      ...
      if (!invites || invites.length === 0) {
        ...
        for (const invite of invites) {
          ...
          inviteCard.className =
            "bg-yellow-50 p-4 rounded-lg flex items-center justify-between";
          ...
          const declineBtn = CommonComponent.createStylizedButton(
            `${language_obj["Decline"]}`,
            "red",
          );
          declineBtn.className = declineBtn.className.replace(
            "py-2 px-4",
            "py-1 px-3 text-sm",
          ); // Make buttons smaller
