...
export class AuthRender {
  ...
  private static createFormElements(container: HTMLElement): {
    ...
    const passwordLabel = CommonComponent.createLabel(
      `${language_obj["Authpage_password_label"]}`,
    );
    const passwordInput = CommonComponent.createInput(
      "password",
      `${language_obj["Authpage_password_input"]}`,
    );
    ...
    const loginButton = CommonComponent.createStylizedButton(
      `${language_obj["Authpage_login_button"]}`,
      "blue",
    );
    const signupButton = CommonComponent.createStylizedButton(
      `${language_obj["Authpage_signup_button"]}`,
      "purple",
    );
  ...
  static show2FAModal(
    ...
    `,
      ...
      // Input
      const input = CommonComponent.createInput(
        "text",
        `${language_obj["Auth2FApage_askcode"]}`,
      );
      ...
      const submitButton = CommonComponent.createStylizedButton(
        `${language_obj["Auth2FApage_submit_button"]}`,
        "blue",
      );
      const cancelButton = CommonComponent.createStylizedButton(
        `${language_obj["Auth2FApage_cancel_button"]}`,
        "gray",
      );
      ...
      submitButton.addEventListener("click", async () => {
        ...
        const shouldClose = await onVerify(code, (err) => {
          msg.textContent = `❌ ${err}`;
        });
        if (shouldClose) {
  ...
  static show2FASetupModal(
    ...
    `);
      ...
      const title = CommonComponent.createHeading(
        `${language_obj["Auth2FApage_Header"]}`,
        2,
        `
      font-['Canada-big'] uppercase font-bold
      text-xl text-center mb-2
    ...
    `,
      ...
      const submitButton = CommonComponent.createStylizedButton(
        `${language_obj["Auth2FApage_verify_button"]}`,
        "blue",
      );
      const cancelButton = CommonComponent.createStylizedButton(
        `${language_obj["Auth2FApage_cancel_button"]}`,
      ...
      submitButton.addEventListener("click", async () => {
        ...
        if (verifyData.success) {
          ...
          msg.textContent =
            `❌ ${verifyData.error}` ||
            `${language_obj["Auth2FApage_error_invalid_code"]}`;
          input.value = "";
        }
      });
  ...
  static showDisplayNameModal(
    ...
    return new Promise((resolve) => {
      ...
      const input = CommonComponent.createInput(
        "text",
        `${language_obj["Enter_display_name"]}`,
      );
      ...
      submitButton.addEventListener("click", async () => {
        ...
        try {
          const availabilityResult =
            await UserService.checkDisplayNameAvailability(displayName);
          ...
          if (availabilityResult.available) {
            ...
            errorMsg.textContent =
              availabilityResult.message || "Display name is already taken";
