...
export class HomeRender {
  ...
  private static renderMainContent(
    ...
    const games = [{ title: "Pong", route: "/game", img: pongImg }] as const;
    ...
    games.forEach(({ title, route, img }) => {
      ...
      canvas.height = 250;
      canvas.className =
        "border-2 border-gray-300 rounded-lg max-w-full h-auto w-[80%]";
  ...
  private static createLeaderboardCard(): HTMLElement {
    ...
    leaderboardContainer.className =
    ...
    title.className =
      "text-white font-bold text-lg mb-3 text-center flex-shrink-0";
  ...
  private static async loadLeaderboardData(
    ...
    try {
      ...
      leaderboard.slice(0, 5).forEach((player: any, index: number) => {
        ...
        playerItem.className =
          "flex items-center justify-between text-white/90 text-sm px-2";
        ...
        name.className =
          "truncate max-w-[120px] cursor-pointer hover:text-blue-400 transition-colors";
        ...
        const totalWins =
          (player.oneVOneWins || 0) + (player.tournamentWins || 0);
