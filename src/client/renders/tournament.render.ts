...
export class TournamentRender {
  ...
  static renderTournamentPage(): {
    ...
    document.body.className =
      "bg-gray-100 font-sans min-h-screen flex flex-col items-center justify-center p-8";
    ...
    const title = CommonComponent.createHeading(
      `${language_obj["Tournamentpage_name_prompt"]}`,
      1,
    ...
    // Player input form
    const formContainer = CommonComponent.createContainer(
    ...
    for (let i = 0; i < 4; i++) {
      const label = CommonComponent.createLabel(
        `${language_obj["Gamepage_player"]} ${i + 1} ${language_obj["Gamepage_name"]}:`,
      );
      const input = CommonComponent.createInput(
        "text",
        `${language_obj["Gamepage_name_input"]} ${i + 1} ${language_obj["Gamepage_name"]}`,
      );
      inputs.push(input);
    ...
    const startButton = CommonComponent.createButton(
