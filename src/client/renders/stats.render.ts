...
export class StatsRender {
	...
	private static createMatchHistorySection(matches: any[], currentUserId: string, user: any): HTMLElement {
		...
		matches.forEach(match => {
			...
			userAvatar.src = user.avatar || '/avatars/default.png';
			...
			opponentAvatar.src = opponent?.avatar || '/avatars/default.png';
		...
		panel.innerHTML = `
            <div class="p-6">
                ...
                <div class="mb-4 p-3 bg-gray-50 rounded-lg flex items-center space-x-3">
                    <img src="${opponent?.avatar || '/avatars/default.png'}" alt="${opponent?.displayName || 'Unknown'}" class="w-12 h-12 rounded-full">
                    <div>
