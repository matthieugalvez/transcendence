...
export class StatsRender {
  private static createStatsHeader(
    user: any,
    isOwnStats: boolean,
  ): HTMLElement {
    ...
    title.textContent = isOwnStats
      ? `${language_obj["Your_stats"]}`
      : `${user.displayName}'s ${language_obj["Stats"]}`;
    ...
    avatar.className =
      "w-14 h-14 rounded-full border-2 border-purple-400 shadow";
  ...
  static async renderStatsContent(
    user: any,
    isOwnStats: boolean,
  ): Promise<void> {
    ...
    const matchesSection = await this.createDetailedMatchesSection(
      user.id,
      user,
    );
    ...
    const tournamentsSection = await this.createTournamentsSection(
      user.id,
      user,
    );
    ...
    const LanguageMenu = CommonComponent.createLanguageMenu(
      language_obj["_lang"],
    );
  ...
  private static createOverviewSection(statsData: any): HTMLElement {
    ...
    const totalGames =
      (statsData.oneVOneWins || 0) +
      (statsData.oneVOneLosses || 0) +
      (statsData.tournamentWins || 0) +
      (statsData.tournamentLosses || 0);
    const totalWins =
      (statsData.oneVOneWins || 0) + (statsData.tournamentWins || 0);
    const winRate =
  ...
  private static createWinLossChart(statsData: any): HTMLElement {
    ...
    const categories = [
      {
        label: `1v1 ${language_obj["Wins"]}`,
        value: statsData.oneVOneWins || 0,
        color: "bg-green-500",
      },
      {
        label: `1v1 ${language_obj["Losses"]}`,
        value: statsData.oneVOneLosses || 0,
        color: "bg-red-500",
      },
      {
        label: `${language_obj["Tournament"]} ${language_obj["Wins"]}`,
        value: statsData.tournamentWins || 0,
        color: "bg-blue-500",
      },
      {
        label: `${language_obj["Tournament"]} ${language_obj["Losses"]}`,
        value: statsData.tournamentLosses || 0,
        color: "bg-orange-500",
      },
    ];
  ...
  private static async createWinRateTimeline(
    userId: string,
  ): Promise<HTMLElement> {
  ...
  private static async createHeadToHeadChart(
    userId: string,
  ): Promise<HTMLElement> {
    const box = document.createElement("div");
    box.className = "bg-gray-50 p-4 rounded-lg";
    ...
    const top3 = Object.entries(stats)
      .sort((a, b) => {
        const aw = a[1].wins / a[1].total,
          bw = b[1].wins / b[1].total;
    ...
    new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { data, backgroundColor: ["#4ade80", "#34d399", "#10b981"] },
        ],
      },
      options: {
        plugins: { legend: { display: false } },
  ...
  private static createWinLossPieChart(stats: any): HTMLElement {
    ...
    const values = [
      stats.oneVOneWins || 0,
      stats.oneVOneLosses || 0,
      stats.tournamentWins || 0,
      stats.tournamentLosses || 0,
    ];
  ...
  private static async createDetailedMatchesSection(
    userId: string,
    user: any,
  ): Promise<HTMLElement> {
    const section = document.createElement("div");
  ...
  private static createMatchHistorySection(
    matches: any[],
    ...
    matches.forEach((match) => {
      ...
      const opponent =
        match.playerOneId === currentUserId ? match.playerTwo : match.playerOne;
      const currentUserScore =
        match.playerOneId === currentUserId
      ...
      if (
        ...
        userAvatar.src = user.avatar.startsWith("/avatars/")
          ? user.avatar
          : `/avatars/${user.avatar}`;
      } else {
      ...
      if (
        ...
        opponent.avatar !== "undefined"
      ) {
        opponentAvatar.src = opponent.avatar.startsWith("/avatars/")
          ? opponent.avatar
          : `/avatars/${opponent.avatar}`;
      } else {
      ...
      matchType.className =
        "text-xs bg-gray-100 px-2 py-1 rounded ml-2 font-medium text-gray-600";
      ...
      result.textContent = isWinner
        ? `${language_obj["Win_iswinner"]}`
        : `${language_obj["Loss_iswinner"]}`;
  ...
  private static showMatchDetails(
    match: any,
    currentUserId: string,
    user: any,
  ): void {
  ...
  private static async fetchRecentMatches(
    userId: string,
    limit: number = 10,
  ): Promise<any[]> {
    try {
  ...
  private static showTournamentDetails(t: any, currentUserId: string) {
    ...
    panel
      .querySelector("#close-match-details")
