...
export class FriendsRender {
    ...
    try {
      ...
      if (pendingIncoming.length > 0) {
        ...
        for (const friendship of pendingIncoming) {
          ...
          const friendCard = await this.createFriendCard(
            friend,
            friendship,
            "pending-received",
          );
      ...
      const pendingOutgoing = friendsList.filter(
        (f) => f.status === "PENDING" && f.senderId === currentUser.id,
      );
  ...
  private static async createFriendCard(
    friend: any,
    friendship: any,
    type: "accepted" | "pending-sent" | "pending-received",
  ): HTMLElement {
    const card = document.createElement("div");
    card.className =
      "bg-gray-50 p-4 rounded-lg flex items-center justify-between hover:bg-gray-100 transition-colors";
    ...
    if (
      ...
      avatar.src = friend.avatar.startsWith("/avatars/")
        ? friend.avatar
    ...
    // Profile button
    const profileBtn = CommonComponent.createStylizedButton(
      `${language_obj["Profile"]}`,
      "purple",
    );
    profileBtn.className = profileBtn.className.replace(
      "py-2 px-4",
      "py-1 px-3 text-sm",
    ); // Smaller button
    profileBtn.onclick = () => {
    ...
    if (type === "pending-received") {
      const acceptBtn = CommonComponent.createStylizedButton(
        `${language_obj["Accept"]}`,
        "blue",
      );
      acceptBtn.className = acceptBtn.className.replace(
        "py-2 px-4",
        "py-1 px-3 text-sm",
      ); // Smaller button
      acceptBtn.onclick = async () => {
        try {
          ...
          CommonComponent.showMessage(
            `❌ ${language_obj["Failed_to_accept_friend_request"]}`,
            "error",
          );
      ...
      const rejectBtn = CommonComponent.createStylizedButton(
        `${language_obj["Reject"]}`,
        "red",
      );
      rejectBtn.className = rejectBtn.className.replace(
        "py-2 px-4",
        "py-1 px-3 text-sm",
      ); // Smaller button
      rejectBtn.onclick = async () => {
        try {
          ...
          CommonComponent.showMessage(
            `❌ ${language_obj["Failed_to_reject_friend_request"]}`,
            "error",
          );
      ...
      const cancelBtn = CommonComponent.createStylizedButton(
        `${language_obj["Cancel"]}`,
        "red",
      );
      cancelBtn.className = cancelBtn.className.replace(
        "py-2 px-4",
        "py-1 px-3 text-sm",
      ); // Smaller button
      cancelBtn.onclick = async () => {
        try {
          ...
          CommonComponent.showMessage(
            `❌ ${language_obj["Failed_to_cancel_friend_request"]}`,
            "error",
          );
      ...
      const removeBtn = CommonComponent.createStylizedButton(
        `${language_obj["Remove"]}`,
        "red",
      );
      removeBtn.className = removeBtn.className.replace(
        "py-2 px-4",
        "py-1 px-3 text-sm",
      ); // Smaller button
      removeBtn.onclick = async () => {
        try {
          ...
          CommonComponent.showMessage(
            `❌ ${language_obj["Failed_to_remove_friend_request"]}`,
          );
      ...
      try {
        ...
        if (invitesResponse.ok) {
          ...
          if (invitesData.success) {
            hasPendingGameInvite = invitesData.invites.some(
              (invite: any) =>
      ...
      if (!hasPendingGameInvite) {
        const inviteBtn = CommonComponent.createStylizedButton(
          `${language_obj["Invite_to_game"]}`,
          "blue",
        );
        inviteBtn.className = inviteBtn.className.replace(
          "py-2 px-4",
          "py-1 px-3 text-sm",
        ); // Smaller button
        ...
        const pendingInviteBtn = CommonComponent.createStylizedButton(
          `${language_obj["Invite_pending"]}`,
          "gray",
        );
        pendingInviteBtn.className = pendingInviteBtn.className.replace(
          "py-2 px-4",
          "py-1 px-3 text-sm",
        ); // Smaller button
  ...
  static showGameTypeModal(friend: any): void {
    ...
    // Duo game button
    const duoBtn = CommonComponent.createStylizedButton(
      `${language_obj["Duo_game"]}`,
      "purple",
    );
    ...
    const tournamentBtn = CommonComponent.createStylizedButton(
      `${language_obj["Tournament_4p"]}`,
      "red",
    );
    ...
    tournamentBtn.onclick = async () => {
      ...
      try {
        ...
        CommonComponent.showMessage(
          "❌ Failed to load tournament selection",
          "error",
        );
      }
    };
  ...
  private static async showTournamentPlayerSelection(
    initialFriend: any,
  ): Promise<void> {
    ...
    try {
      ...
      if (availableFriends.length < 2) {
        ...
        CommonComponent.showMessage(
          `❌ ${language_obj["You_need_3_friends"]}`,
          "error",
        );
        return;
      }
      ...
      const currentUserPill = FriendsRender.createPlayerPill(
        currentUser.displayName,
        false,
      );
      const initialFriendPill = FriendsRender.createPlayerPill(
        ...
        checkbox.onchange = () => {
          if (checkbox.checked) {
            if (selectedFriends.length >= 2) {
              ...
              CommonComponent.showMessage(
                `❌ ${language_obj["You_can_only_select_2more"]}`,
                "error",
              );
              return;
            }
            selectedFriends.push(friend);
            const pill = FriendsRender.createPlayerPill(
              friend.displayName,
              true,
              () => {
              ...
              friend.id,
            ); // Pass the friend.id as playerId parameter
            selectedList.appendChild(pill);
          } else {
            selectedFriends = selectedFriends.filter((f) => f.id !== friend.id);
            const pill = selectedList.querySelector(
              `[data-player-id="${friend.id}"]`,
            );
            if (pill) pill.remove();
          }
          updateStartButton();
      ...
      const startBtn = CommonComponent.createStylizedButton(
        `${language_obj["Tournamentpage_start"]}`,
        "purple",
      );
      ...
      CommonComponent.showMessage(
        `❌ ${language_obj["Failed_to_load_friends_list"]}`,
        "error",
      );
    }
  }
  ...
  private static createPlayerPill(
    displayName: string,
    removable: boolean,
    onRemove?: () => void,
    playerId?: string,
    ...
    if (removable && onRemove) {
      ...
      removeBtn.className =
        "ml-2 text-purple-600 hover:text-purple-800 font-bold";
  ...
  private static async sendTournamentInvites(players: any[]): Promise<void> {
    try {
      ...
      if (failedInvites.length > 0) {
        CommonComponent.showMessage(
          `⚠️ ${language_obj["Some_invites_failed"]}`,
          "warning",
        );
      } else {
        CommonComponent.showMessage(
          `✅ ${language_obj["Tournament_invites_sent"]}`,
          "success",
        );
        ...
        const chatPromises = players.map(async (player) => {
          try {
            ...
            if (result.success) {
              ...
              console.error(
                `❌ Failed to send tournament message to ${player.displayName}:`,
                result.error,
              );
            }
            return result;
      ...
      CommonComponent.showMessage(
        `❌ ${language_obj["Failed_to_create_tournament"]}`,
        "error",
      );
    }
  }

  private static async sendGameInvite(
    friend: any,
    gameType: "duo" | "tournament",
  ): Promise<void> {
    try {
      ...
      if (friend.id === currentUser?.id) {
        CommonComponent.showMessage(
          `❌ ${language_obj["Cannot_invite_self"]}`,
          "error",
        );
        return;
      }
      ...
      const endpoint =
        gameType === "duo" ? "/api/game/start" : "/api/game/tournament/start";
      ...
      CommonComponent.showMessage(
        `✅ ${gameType === "duo" ? "Duo" : `${language_obj["Tournament"]}`} ${language_obj["Invite_sent_to"]} ${friend.displayName}!`,
        "success",
      );
      ...
      CommonComponent.showMessage(
        `❌ ${error.message || `${language_obj["Failed_to_send"]} ${gameType} ${language_obj["Invite"]}`}`,
        "error",
      );
    }
  }
