...
function showViewportWarning(): void {
  ...
  message.textContent =
    "Transcendence requires a minimum screen size of 1024x768 pixels for the best gaming experience.";
...
function startSPA() {
  ...
  router.register("/game/online/duo/:gameId", (params = {}) =>
    renderJoinPage({ gameId: params.gameId, mode: "duo" }),
  );
  router.register("/game/online/tournament/:gameId", (params = {}) =>
    renderJoinPage({ gameId: params.gameId, mode: "tournament" }),
  );
  router.register("/profile", async () => await StatsPage());
  router.register(
    "/profile/:displayName",
    async (params = {}) => await StatsPage({ displayName: params.displayName }),
  );
