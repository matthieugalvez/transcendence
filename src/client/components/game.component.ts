...
export class GameSettingsComponent {
  ...
  static render(
    ...
    if (state === "solo") {
      // Play/Pause/Restart
      settingsBar.appendChild(
        GameSettingsComponent.renderPlayPauseRestart(callbacks),
      );

      // Bouton Start
      const startBtn = CommonComponent.createStylizedButton(
        `${language_obj["Gamepage_start_button"]}`,
        "red",
      );
      ...
      startBtn.onclick = () =>
        callbacks.onStartGame?.(
          "solo",
          GameSettingsComponent.currentDifficulty,
        );
    ...
    if (state === "duo") {
      ...
      const onlineBtn = CommonComponent.createStylizedButton(
        `${language_obj["Online"]}`,
        "orange",
      );
    ...
    if (state === "duo-start") {
      ...
      // Difficulté
      settingsBar.appendChild(
        GameSettingsComponent.renderDifficultyBtns(callbacks, true),
      );
    }
    ...
    if (state === "duo-local") {
      ...
      settingsBar.appendChild(GameSettingsComponent.renderGuide("duo"));
      settingsBar.appendChild(
        GameSettingsComponent.renderDifficultyBtns(callbacks),
      );
    }
    ...
    if (state === "duo-online") {
      ...
      const copyBtn = CommonComponent.createStylizedButton(
        `${language_obj["Gamepage_game_link"]}`,
        "orange",
      );
      copyBtn.onclick = () => {
        const link = callbacks.getOnlineLink?.() ?? "";
        ...
        if (navigator.clipboard && window.isSecureContext) {
          ...
          navigator.clipboard
            ...
            .then(() => {
              ...
              setTimeout(
                () =>
                  (copyBtn.textContent = `${language_obj["Gamepage_game_link"]}`),
                1200,
              );
            })
            .catch((err) => {
              ...
              GameSettingsComponent.fallbackCopyTextToClipboard(
                link,
                copyBtn,
                callbacks,
              );
    ...
    if (state === "tournament-alias") {
      ...
      title.className =
        'font-["Canada-big"] capitalize mb-4 text-white text-2xl justify-center items-center';
      settingsBar.appendChild(title);
      ...
      // Bouton Start
      const startButton = CommonComponent.createStylizedButton(
  ...
  static fallbackCopyTextToClipboard(
    text: string,
    ...
    try {
      ...
      if (successful) {
        ...
        setTimeout(
          () => (button.textContent = `${language_obj["Gamepage_game_link"]}`),
          1200,
        );
      } else {
  ...
  static renderPlayPauseRestart(callbacks: GameSettingsCallbacks) {
    ...
    setBox.className =
      "flex flex-row items-center justify-center space-x-8 mb-4";
  ...
  static renderGuide(mode: "solo" | "duo") {
    ...
    const makeSide = (
      ...
      title.className =
        'font-["Orbitron"] text-white text-center whitespace-normal';
