...
export class UserComponent {
  static async saveSettings(): Promise<void> {
    ...
    const msgDisplay = document.querySelector(
      "#signup-msg-display",
    ) as HTMLElement;
   ...
   */
  private static handleUserUpdateError(
    apiResponseData: any,
    fieldName: string,
    errors: string[],
  ): void {
    if (apiResponseData.details && apiResponseData.details.length > 0) {
      ...
      const validationErrors = apiResponseData.details.map(
        (detail: any) => `❌ ${fieldName} ${detail.message}`,
      );
      ...
      const errorMessage =
        ...
        `${language_obj["Authpage_update_error"]} ${fieldName.toLowerCase()}`;
