  ...
  static async showPlayerSelection(
    ...
    for (let i = 0; i < 4; i++) {
      ...
      function renderSearchUI() {
        ...
        UserSearchComponent.render(
          searchSection,
          (user) => {
            // verif si deja selectionner
            const already = slots.some(
              (s, idx) => idx !== i && s.user === user.displayName,
            );
            if (already) {
              CommonComponent.showMessage(
                `${language_obj["Tournamentpage_already_selected"]}`,
                "error",
              );
              return;
            }
      ...
      function renderSelectedUI(user: {
        displayName: string;
        avatar: string;
        id: string;
      }) {
        ...
        pill.className =
          "flex items-center justify-between p-2 bg-purple-900/60 text-white rounded";
        pill.innerHTML = `
        ...
        deselectBtn.className =
          "ml-4 px-2 py-1 bg-red-600 rounded hover:bg-red-700";
  ...
  static showTransitionPanel(
    ...
    if (i < matchups.length - 1) {
      const [nextLeft, nextRight] =
        i + 1 === 2 ? [winners[0], winners[1]] : matchups[i + 1];
