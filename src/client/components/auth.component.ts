...
export class AuthComponent {
  ...
  static async signupUser(email: string, password: string): Promise<boolean> {
    if (!AuthService.validateInput(email, password)) {
      CommonComponent.showMessage(
        `${language_obj["Authpage_error_empty_field"]}`,
        "error",
      );
      return false;
    }
  ...
  static async loginUser(
    ...
    const apiResponseData = await AuthService.loginUser(
      email,
      password,
      twoFACode,
    );
    ...
    if (apiResponseData.success) {
      ...
      const twoFAErrors = [
        `${language_obj["Auth2FApage_error_empty_field"]}`,
      ...
      if (
        ...
        CommonComponent.showMessage(
          `❌ ${apiResponseData.error || language_obj["Authpage_error_login_failed"]}`,
          "error",
        );
      }
      return apiResponseData;
  ...
  static async logoutUser(): Promise<boolean> {
    ...
    if (apiResponseData.success) {
      ...
      CommonComponent.showMessage(
        `❌ ${apiResponseData.error || `${language_obj["Authpage_error_logout_failed"]}`}`,
        "error",
      );
      return false;
    }
  ...
  private static handleAuthError(apiResponseData: any): void {
    let errorMessage =
      ...
      `${language_obj["Authpage_error_registration_failed"]}`;
  ...
  static async SetLanguageUser(language: string): Promise<boolean> {
    ...
    if (apiResponseData.success) {
      ...
      CommonComponent.showMessage(
        `❌ ${apiResponseData.error || language_obj["Language_change_failed"]}`,
        "error",
      );
      return false;
    }
  ...
  static async handle2FASetup() {
    ...
    if (!data.success) {
      CommonComponent.showMessage(
        `❌ ${data.error || language_obj["Auth2FApage_error"]}`,
        "error",
      );
      return;
    }
    ...
    while (true) {
      const code = await AuthRender.show2FASetupModal(
      ...
      if (verifyData.success) {
        CommonComponent.showMessage(
          `✅ ${language_obj["Auth2FAPage_enabled"]}`,
          "success",
        );
        break;
      } else {
  ...
  static async disable2FA(): Promise<boolean> {
    ...
    if (apiResponseData.success) {
      CommonComponent.showMessage(
        `✅ ${language_obj["Auth2FAPage_disabled"]}`,
        "success",
      );
      return true;
    } else {
      CommonComponent.showMessage(
        `❌ ${apiResponseData.error || language_obj["Auth2FApage_disable_error"]}`,
        "error",
      );
      return false;
    }
  ...
  static async signupUserWithDisplayName(
    email: string,
    password: string,
    displayName: string,
  ): Promise<boolean> {
    ...
    try {
      ...
      if (data.success) {
        CommonComponent.showMessage(
          `✅ ${data.message || language_obj["Authpage_account_creation_success"]}`,
          "success",
        );
        return true;
      } else {
      ...
      CommonComponent.showMessage(
        `❌ ${language_obj["Authpage_account_creation_failure"]}`,
        "error",
      );
      return false;
    }
  ...
  static async checkAndHandleDisplayName(): Promise<{
    success: boolean;
    userData?: any;
  }> {
    try {
      ...
      if (updateResult.success) {
        ...
        let errorMessage =
          updateResult.error || `${language_obj["Display_name_failure"]}`;
