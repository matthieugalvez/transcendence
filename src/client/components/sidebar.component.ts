...
export class SidebarComponent {
  static async render(opts: SidebarOptions) {
    ...
    if (showFriendsBtn) {
      ...
      const friendsBtn = CommonComponent.createStylizedButton(
        `👥 ${language_obj["Sidebar_friendlist"]}`,
        "purple",
      );
      ...
      notificationBell.className =
        "absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs font-bold border-2 border-white";
      ...
      friendsBtnContainer.setAttribute(
        "data-interval-id",
    ...
    // Back to home button
    if (showBackHome) {
      const backButton = CommonComponent.createStylizedButton(
        `🔙 ${language_obj["Sidebar_home"]}`,
        "purple",
      );
    ...
    // Settings button
    if (showSettings) {
      const settingBtn = CommonComponent.createStylizedButton(
        `⚙️ ${language_obj["Sidebar_settings"]}`,
        "blue",
      );
    ...
    // Logout button
    const logoutBtn = CommonComponent.createStylizedButton(
  ...
  private static async checkForPendingRequests(): Promise<boolean> {
    try {
      ...
      const pendingFriendRequests = friendsList.filter(
