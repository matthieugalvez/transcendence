...
export class TournamentRoom {
  ...
  async addClient(
    ws: WebSocket,
    username?: string,
  ): Promise<number | "spectator" | "already_joined"> {
    if (username) {
      const existingPlayer = this.players.find(
        (p) => p.username === username && p.ws,
      );
      if (existingPlayer) {
  ...
  private startMatch() {
    ...
    this.currentGame.onEnd(
      async (winnerId) => await this.handleMatchEnd(winnerId),
    );
    ...
    spectators.forEach((s) => {
      if (s.ws) this.currentGame!.addSpectator(s.ws);
    });
  ...
  private async handleMatchEnd(winnerId: number) {
    ...
    const winner =
      winnerId === 1 ? this.currentPlayers[0] : this.currentPlayers[1];
    ...
    if (this.currentMatch < 2) {
      ...
      this.broadcastAll(
        JSON.stringify({ type: "tournamentEnd", winner: winner.username }),
      );
    }
  }
  ...
  forwardMessage(playerSlot: number, msg: any) {
    ...
    if (msg.action === "up" || msg.action === "down") {
      ...
      if (!this.currentGame.getCurrentState().isRunning)
  ...
  private async sendRoundNotification(): Promise<void> {
    try {
      ...
      const messagePromises = this.players
        ...
        .map(async (player) => {
          try {
            ...
            console.log(
              `✅ Round notification sent from ${this.players[0].username} to ${player.username}`,
            );
          } catch (error) {
            console.error(
              `❌ Failed to send round notification to ${player.username}:`,
              error,
            );
          }
        });
      ...
      console.log(
        `🎯 Round ${this.currentMatch} notifications sent to all players`,
      );
    } catch (error) {
      ...
      const messagePromises = this.players
        ...
        .map(async (player) => {
          try {
            ...
            console.log(
              `✅ Tournament start notification sent from ${this.players[0].username} to ${player.username}`,
            );
          } catch (error) {
            console.error(
              `❌ Failed to send tournament start notification to ${player.username}:`,
              error,
            );
          }
        });
