...
export class GameInstance {
  ...
  ): number | "spectator" | "already_joined" {
          ...
          this.waitingTimeout = setTimeout(() => {
            if (this.getConnectedPlayersCount() < 2 && !this.isRunning) {
              ...
              const payload = JSON.stringify({
                ...
                reason: "timeout",
                message:
                  "No other player joined within 1 minutes. Game cancelled.",
                shouldRedirect: true,
              });
  ...
  public start() {
    ...
    this.countdownInterval = setInterval(() => {
      s--;
      if (s >= 0)
        this.broadcastToAll(JSON.stringify({ type: "countdown", seconds: s }));
      else {
  ...
  public tryReconnectPlayer(token: string, ws: WebSocket): boolean {
    ...
    ws.send(
      JSON.stringify({
        type: "resume",
        message: "You have reconnected. Game resumes.",
      }),
    );
    ws.send(
      JSON.stringify({
  ...
  private checkCollisions() {
    ...
    if (
      this.ballPos.x - this.ballRadius <=
        this.paddle1Pos.x + this.paddleWidth &&
      ...
      // Nouvelle vitesse
      const speed =
        Math.sqrt(this.ballVel.vx ** 2 + this.ballVel.vy ** 2) *
    ...
    if (
      ...
      this.ballPos.x - this.ballRadius <=
        this.paddle2Pos.x + this.paddleWidth &&
  ...
  private resetPaddles() {
    this.paddle1Pos.y = this.paddle2Pos.y =
      (this.canvasHeight - this.paddleHeight) / 2;
  ...
  private startPauseOnDisconnect() {
    ...
    this.pauseTimeoutHandle = setTimeout(() => {
      if (!this.players.every((p) => p.ws)) {
        ...
        console.log(
          `Timeout fired but all players are connected for game ${this.gameId}`,
        );
      }
    }, 30000); // 30 sec
  }
  ...
  private cancelPauseOnReconnect() {
    ...
    if (this.players.every((p) => p.ws)) {
      this.isPaused = true;
      this.broadcastPause(
        "Both players reconnected. Host can restart the game.",
      );
      this.broadcastState(this.isRunning);
    }
  }
  ...
  private async handleInviterLeave(): Promise<void> {
    ...
    try {
      const { GameCleanupService } =
        await import("../services/gamecleanup.service.js");
      await GameCleanupService.cleanupGameAndInvites(
        this.gameId,
        this.inviterId!,
      );
    } catch (error) {
