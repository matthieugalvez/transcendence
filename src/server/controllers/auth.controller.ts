...
export class AuthController {
  static async signup(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      console.log(
        "🔑 AccessToken being set:",
        accessToken ? "***CREATED***" : "FAILED",
      );
  ...
  static async login(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { email, password, twoFACode } = request.body as {
        email: string;
        password: string;
        twoFACode: string;
      };
      ...
      console.log(
        "🔄 RefreshToken being set:",
        refreshToken ? "***CREATED***" : "FAILED",
      );
  ...
  static async logout(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      console.log(
        "🍪 Cookies cleared and refresh token removed from database for user:",
        userId,
      );
  ...
  static async verify2FA(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (!isValid) return Send.unauthorized(reply, "Invalid 2FA code");
    ...
    try {
      ...
      if (!googleAccessToken) {
        ...
        return reply.redirect(
          getAuthRedirectUrl("/auth?error=no_google_token"),
        );
      }
      ...
      if (!response.ok) {
        ...
        return reply.redirect(
          getAuthRedirectUrl("/auth?error=invalid_google_token"),
        );
      }
  ...
  static async googleSignin(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (!oauth2) {
        ...
        console.log(
          "Available plugins on server:",
          Object.keys(request.server),
        );
        return reply.redirect(
          getAuthRedirectUrl("/auth?error=oauth_not_configured"),
        );
      }
      ...
      return reply.redirect(
        getAuthRedirectUrl("/auth?error=google_signin_failed"),
      );
    }
  }

  // Check le token JWT pendant le OAuth si le 2FA Est actif.
  static async checkOAuth2FAStatus(
    ...
    try {
      ...
      try {
        const decoded = jwt.verify(tempOAuthAuth, authConfig.secret) as {
          userId: string;
          purpose: string;
        };
  ...
  // DEPRECATED
  static async signupWithDisplayName(
    request: FastifyRequest,
    reply: FastifyReply,
  ) {
    try {
      ...
      const isDisplayNameTaken =
        await UserService.isDisplayNameTaken(displayName);
      ...
      console.log(
        "🍪 Setting cookies for new user with display name:",
        user.id,
      );
