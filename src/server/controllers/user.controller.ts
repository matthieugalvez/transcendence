...
export class UserController {
  ...
  static async checkUserExists(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const user = await UserService.getUserByDisplayName(
  ...
  static async checkDisplayNameAvailability(
    ...
    try {
      ...
      const isDisplayNameTaken = await UserService.isDisplayNameTaken(
        displayName.trim(),
        userId,
      );
      ...
      return Send.internalError(
        reply,
        "Failed to check display name availability",
      );
    }
  }
  ...
  static async searchUsers(request: FastifyRequest, reply: FastifyReply) {
    try {
      const userId = (request as any).userId;
      const { q: query, limit = 10 } = request.query as {
        q: string;
        limit?: number;
      };
