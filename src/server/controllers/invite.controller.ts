...
export class InviteController {
  static async getSentInvites(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      return Send.internalError(
        reply,
        error?.message || "Failed to get sent invites",
      );
    }
  }
  ...
  static async createInvite(request: FastifyRequest, reply: FastifyReply) {
    try {
      const inviterId =
        ...
        ((request as any).user && (request as any).user.id);
      ...
      if (error?.message === "Cannot invite yourself") {
        return reply
          .code(400)
          .send({ success: false, error: "Cannot invite yourself" });
      }
      if (
        error?.message === "User already has a pending invite for this game"
      ) {
        return reply
          .code(409)
          .send({ success: false, error: "User already invited to this game" });
      }

      reply
        .code(500)
        .send({ success: false, error: "Failed to create invite" });
    }
  }
  ...
  static async getPendingInvites(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      return Send.internalError(
        reply,
        error?.message || "Failed to get invites",
      );
    }
  }
