...
export function handlePongWebSocket(connection: any, req: any) {
  ...
  ws.on('message', (data: string) => {
    try {
      const parsed = JSON.parse(data) as { playerId: number; action: 'up' | 'down' };
      game!.onClientAction(parsed.playerId, parsed.action);
    } catch (err) {
