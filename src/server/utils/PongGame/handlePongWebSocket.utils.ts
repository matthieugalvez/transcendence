...
import {
  ...
  getGameRoom,
  createGameRoom,
} from "../../game/gameRooms.js"; // Add createGameRoom import
import { GameInstance } from "../../game/gameInstance.js"; // Add .js extension
import {
  getTournamentRoom,
  createTournamentRoom,
  removeTournamentRoom,
} from "../../game/tournamentRooms.js";
...
function attachTournamentHandler(
  ws: WebSocket,
  tour: any,
  playerId: number | "spectator",
) {
  ws.on("message", (data: string) => {
    try {
      const msg = JSON.parse(data);
  ...
  if (ws.readyState === ws.OPEN) {
    ws.send(
      JSON.stringify({
        type: "playersJoined",
        players: tour.getConnectedPlayerIds(),
      }),
    );
  }
}
...
export async function handlePongWebSocket(ws: WebSocket, req: any) {
  ...
  const userWasInvited = invites.some(
    (invite) =>
      invite.invitee?.displayName?.trim().toLowerCase() ===
  ...
  const userIsCreator = invites.some(
    (invite) =>
      invite.inviter?.displayName?.trim().toLowerCase() ===
      username?.trim().toLowerCase(),
  );
  ...
  if (userWasInvited && !userIsCreator) {
    ...
    if (!userInvite) {
      ...
      const allUserInvites = invites.filter(
        (i) =>
          i.invitee?.displayName?.trim().toLowerCase() ===
          username?.trim().toLowerCase(),
      );
      console.log(
        "All invites for this user:",
        allUserInvites.map((i) => ({
          status: i.status,
          expiresAt: i.expiresAt,
        })),
      );
      ...
      ws.send(
        JSON.stringify({
          ...
          message:
            "Your invite has expired. You have been removed from the game.",
        }),
      );
      ws.close();
  ...
  if (mode === "tournament") {
    ...
    ws.send(
      JSON.stringify({ type: "playerToken", playerId: role, playerToken: "" }),
    );
  ...
  if (!game) {
    ...
    const inviterId =
      gameInvites.length > 0 ? gameInvites[0].inviterId : undefined;
  ...
  ws.on("close", () => {
    ...
    if (game && username) {
      const userIsInviter = invites.some(
        (invite) =>
          invite.inviter?.displayName?.trim().toLowerCase() ===
          username?.trim().toLowerCase(),
      );
      ...
      if (userIsInviter && !game.getCurrentState().isRunning) {
        console.log(
          `Inviter ${username} left before game started, triggering cleanup`,
        );
        const inviterInvite = invites.find(
          (invite) =>
            invite.inviter?.displayName?.trim().toLowerCase() ===
            username?.trim().toLowerCase(),
        );
        if (inviterInvite) {
          import("../../services/gamecleanup.service.js")
            .then(({ GameCleanupService }) => {
              GameCleanupService.cleanupGameAndInvites(
                gameId,
                inviterInvite.inviterId,
              );
            })
            .catch((error) => {});
        }
