...
export class StatsService {
  ...
  static async incrementLoss(
    userId: string,
    matchType: "ONE_V_ONE" | "TOURNAMENT",
  ) {
    const field =
      matchType === "ONE_V_ONE" ? "oneVOneLosses" : "tournamentLosses";
    ...
    try {
      ...
      return leaderboard.map((stats) => ({
        ...
        avatar: stats.user.avatar
          ? stats.user.avatar.startsWith("/avatars/")
            ? stats.user.avatar
            : `/avatars/${stats.user.avatar}`
          : "default.svg",
